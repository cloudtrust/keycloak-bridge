...
const (
	...
	updateThemeConfigStmt = `
		INSERT INTO client_config (theme_name, color, menu_theme, font_family, logo, favicon, translation_en, translation_de, translation_fr, translation_it)
		VALUES (?,?,?,?,?,?,?,?,?,?)
	...
	`
	selectTranslationStmt = `SELECT ??? FROM client_config WHERE theme_name = ?;`
)
