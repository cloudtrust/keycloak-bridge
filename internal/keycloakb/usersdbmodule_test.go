...
func TestCreateCheck(t *testing.T) {
	...
	t.Run("Create check successful", func(t *testing.T) {
		mockDB.EXPECT().Exec(createCheckStmt, realm, userID, gomock.Any(), gomock.Any(),
			...
			gomock.Any(), realm, userID, gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(),
...
func TestGetPendingChecks(t *testing.T) {
	...
	t.Run("Unexpected error", func(t *testing.T) {
		mockDB.EXPECT().Query(selectPendingCheckStmt, realm, userID).Return(mockSQLRows, unexpectedError)

		var _, err = usersDBModule.GetPendingChecks(ctx, realm, userID)
		assert.Equal(t, unexpectedError, err)
	})

	t.Run("No row", func(t *testing.T) {
		mockDB.EXPECT().Query(selectPendingCheckStmt, realm, userID).Return(mockSQLRows, sql.ErrNoRows)

		var checks, err = usersDBModule.GetPendingChecks(ctx, realm, userID)
		assert.Nil(t, err)
		assert.Nil(t, checks)
	})

	t.Run("Can't fetch result", func(t *testing.T) {
		mockDB.EXPECT().Query(selectPendingCheckStmt, realm, userID).Return(mockSQLRows, nil)
		mockSQLRows.EXPECT().Next().Return(true)
		mockSQLRows.EXPECT().Scan(gomock.Any()).Return(unexpectedError)
		mockSQLRows.EXPECT().Close()
