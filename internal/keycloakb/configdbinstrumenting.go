...
type ConfigurationDBModule interface {
	...
	GetThemeTranslation(ctx context.Context, themeName string, language string) (string, error)
}
...
func (m *configDBModuleInstrumentingMW) GetThemeTranslation(ctx context.Context, themeName string, language string) (string, error) {
	defer func(begin time.Time) {
		m.h.With(KeyCorrelationID, ctx.Value(cs.CtContextCorrelationID).(string)).Observe(time.Since(begin).Seconds())
	}(time.Now())
	return m.next.GetThemeTranslation(ctx, themeName, language)
}
