...
func TestToDetails(t *testing.T) {
	...
	now := time.Now().Unix()
	expiry := "31.12.2099"
	accred := AccreditationRepresentation{
		Type:           &accredType,
		CreationMillis: &now,
		ExpiryDate:     &expiry,
	}
...
func TestHasActiveAccreditations(t *testing.T) {
	...
	t.Run("only revoked accreditation", func(t *testing.T) {
		accreds := []string{
			`{"type":"ONE","expiryDate":"01.01.2015","revoked":"true"}`,
	...
	t.Run("only expired accreditation", func(t *testing.T) {
		accreds := []string{`{"type":"ONE","expiryDate":"01.01.2015"}`}
		ap, _ := NewAccreditationsProcessor(accreds)
		assert.False(t, ap.HasActiveAccreditations())
	})
...
func TestRevokeAccreditations(t *testing.T) {
	...
	t.Run("No existing accreditations", func(t *testing.T) {
		...
		updated := RevokeAccreditations(&user, func(AccreditationRepresentation) {})
		assert.Len(t, user.GetAttribute(constants.AttrbAccreditations), 0)
		assert.False(t, updated)
	})
	t.Run("Nothing changes", func(t *testing.T) {
		user := kc.UserRepresentation{}
		accreds := []string{"a", "b", `{"type":"ONE", "expiryDate":"01.01.2015", "creationMillis":123456789, "revoked":true}`}
...
func TestAddAccreditation(t *testing.T) {
	ap, _ := NewAccreditationsProcessor(nil)
