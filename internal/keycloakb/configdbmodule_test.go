...
func TestThemeConfiguration(t *testing.T) {
	...
	t.Run("UpdateThemeConfiguration", func(t *testing.T) {
		...
		t.Run("Success", func(t *testing.T) {
			mockDB.EXPECT().Exec(updateThemeConfigStmt, ptr(themeName), settingsJSON, &logo, &favicon, translationsJSON).Return(nil, nil)
			err := configDBModule.UpdateThemeConfiguration(ctx, themeConfig)
			assert.Nil(t, err)
		})

		t.Run("SQL error", func(t *testing.T) {
			mockDB.EXPECT().Exec(updateThemeConfigStmt, ptr(themeName), settingsJSON, &logo, &favicon, translationsJSON).Return(nil, sqlError)
			err := configDBModule.UpdateThemeConfiguration(ctx, themeConfig)
			assert.Equal(t, sqlError, err)
		})
	})
