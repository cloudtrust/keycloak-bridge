...
func (c *component) GetIdentityProviderMappers(ctx context.Context, realmName string, idpAlias string) ([]api.IdentityProviderMapperRepresentation, error) {
	...
	apiMappers := make([]api.IdentityProviderMapperRepresentation, len(kcMappers))
	for i := range kcMappers {
		apiMappers[i] = api.ConvertToAPIIdentityProviderMapper(kcMappers[i])
	}

	return apiMappers, nil
}
