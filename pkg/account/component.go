...
type Component interface {
	...
	CanIdentify(ctx context.Context, contextKey *string) (bool, error)
}
...
func (c *component) GetConfiguration(ctx context.Context, realmIDOverride string) (api.Configuration, error) {
	...
	var apiConfig = api.Configuration{
		...
		VideoIdentificationAllowedRoles:          adminConfig.VideoIdentificationAllowedRoles,
		AuxiliaryVideoIdentificationAllowedRoles: adminConfig.AuxiliaryVideoIdentificationAllowedRoles,
		AutoIdentificationAllowedRoles:           adminConfig.AutoIdentificationAllowedRoles,
		PhysicalIdentificationAllowedRoles:       adminConfig.PhysicalIdentificationAllowedRoles,
...
func (c *component) CanIdentify(ctx context.Context, contextKey *string) (bool, error) {
	var accessToken = ctx.Value(cs.CtContextAccessToken).(string)
	...
	for _, accred := range accreditations {
		if accred.Type != nil && *accred.Type == accreditationType && !*accred.Revoked {
			...
			if accred.ExpiryDate != nil {
				if expiry, err := time.Parse(constants.SupportedDateLayouts[0], *accred.ExpiryDate); err == nil && expiry.After(latestExpiration) {
