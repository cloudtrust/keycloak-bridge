...
func TestGetCredentialsForUser(t *testing.T) {
	...
	{
		mockKeycloakClient.EXPECT().GetCredentialsForUser(accessToken, realmName, userID).Return([]api.CredentialRepresentation{}, nil).Times(1)
...
func TestDeleteCredentialsForUser(t *testing.T) {
	...
	// Get credentials for user
	{
		mockKeycloakClient.EXPECT().DeleteCredentialsForUser(accessToken, realmName, userID, credential).Return(nil).Times(1)
