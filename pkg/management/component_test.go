...
func testThemeConfiguration() api.ThemeConfiguration {
...
func TestGetThemeConfiguration(t *testing.T) {
	...
	t.Run("Get theme configuration success", func(t *testing.T) {
		var expectedConfig = testThemeConfiguration()
		mocks.configurationDBModule.EXPECT().GetAdminConfiguration(ctx, realmName).Return(configuration.RealmAdminConfiguration{BoTheme: &themeName}, nil)
		mocks.configurationDBModule.EXPECT().GetThemeConfiguration(ctx, themeName).Return(configuration.ThemeConfiguration{
			Color:      ptr("#000000"),
			MenuTheme:  ptr("dark"),
			FontFamily: ptr("Lato"),
		}, nil)
	...
	t.Run("Get theme configuration error no bo theme", func(t *testing.T) {
		mocks.configurationDBModule.EXPECT().GetAdminConfiguration(ctx, realmName).Return(configuration.RealmAdminConfiguration{BoTheme: nil}, nil)
		config, err := managementComponent.GetThemeConfiguration(ctx, realmName)
		assert.NotNil(t, err)
	...
	t.Run("Get theme configuration error db", func(t *testing.T) {
		var expectedError = errors.New("db error")
		mocks.configurationDBModule.EXPECT().GetAdminConfiguration(ctx, realmName).Return(configuration.RealmAdminConfiguration{BoTheme: &themeName}, nil)
		mocks.configurationDBModule.EXPECT().GetThemeConfiguration(ctx, themeName).Return(configuration.ThemeConfiguration{}, expectedError)
		config, err := managementComponent.GetThemeConfiguration(ctx, realmName)
		assert.NotNil(t, err)
		assert.Equal(t, api.ThemeConfiguration{}, config)
	})
...
func TestUpdateThemeConfiguration(t *testing.T) {
	...
	var translationBase64 = base64.StdEncoding.EncodeToString([]byte(translationEN))
	configurationThemeConfig := api.UpdatableThemeConfiguration{
		ThemeName:     &themeName,
		Color:         ptr("#000000"),
		MenuTheme:     ptr("dark"),
		FontFamily:    ptr("Lato"),
		TranslationEN: &translationBase64,
	}
	...
	t.Run("Update theme translation file not base64 encoded", func(t *testing.T) {
		var newConfig = configurationThemeConfig
		newConfig.TranslationEN = ptr("{\"key\": \"value\"}")
		err := managementComponent.UpdateThemeConfiguration(ctx, realmName, newConfig)
		assert.NotNil(t, err)
	})
}
...
func TestGetThemeTranslation(t *testing.T) {
	...
	var expectedTranslation = map[string]interface{}{
		"key":     "value",
		"welcome": "Welcome",
	}
	...
	t.Run("Get theme translation success", func(t *testing.T) {
		mocks.configurationDBModule.EXPECT().GetAdminConfiguration(ctx, realmName).Return(configuration.RealmAdminConfiguration{BoTheme: &themeName}, nil)
		mocks.configurationDBModule.EXPECT().GetThemeTranslation(ctx, themeName, language).Return(translationJSON, nil)
	...
	t.Run("Get admin configuration fails", func(t *testing.T) {
		var expectedError = errors.New("db error")
		mocks.configurationDBModule.EXPECT().GetAdminConfiguration(ctx, realmName).Return(configuration.RealmAdminConfiguration{}, expectedError)
		translation, err := managementComponent.GetThemeTranslation(ctx, realmName, language)
		assert.Equal(t, expectedError, err)
	...
	t.Run("Get theme translation from DB fails", func(t *testing.T) {
		var expectedError = errors.New("db error")
		mocks.configurationDBModule.EXPECT().GetAdminConfiguration(ctx, realmName).Return(configuration.RealmAdminConfiguration{BoTheme: &themeName}, nil)
		mocks.configurationDBModule.EXPECT().GetThemeTranslation(ctx, themeName, language).Return("", expectedError)
		translation, err := managementComponent.GetThemeTranslation(ctx, realmName, language)
		assert.Equal(t, expectedError, err)
		assert.Equal(t, "", translation)
	})
	...
	t.Run("Invalid JSON format", func(t *testing.T) {
		...
		mocks.configurationDBModule.EXPECT().GetAdminConfiguration(ctx, realmName).Return(configuration.RealmAdminConfiguration{BoTheme: &themeName}, nil)
		mocks.configurationDBModule.EXPECT().GetThemeTranslation(ctx, themeName, language).Return(invalidJSON, nil)
		translation, err := managementComponent.GetThemeTranslation(ctx, realmName, language)
		assert.NotNil(t, err)
	...
	t.Run("Different languages", func(t *testing.T) {
		...
		var expectedTranslations = map[string]interface{}{
			"DE": map[string]interface{}{"schlüssel": "wert"},
			"FR": map[string]interface{}{"clé": "valeur"},
			"IT": map[string]interface{}{"chiave": "valore"},
		}

		for _, lang := range languages {
			mocks.configurationDBModule.EXPECT().GetAdminConfiguration(ctx, realmName).Return(configuration.RealmAdminConfiguration{BoTheme: &themeName}, nil)
			mocks.configurationDBModule.EXPECT().GetThemeTranslation(ctx, themeName, lang).Return(translations[lang], nil)
	...
	t.Run("Empty translation JSON", func(t *testing.T) {
		var emptyJSON = `{}`
		mocks.configurationDBModule.EXPECT().GetAdminConfiguration(ctx, realmName).Return(configuration.RealmAdminConfiguration{BoTheme: &themeName}, nil)
		mocks.configurationDBModule.EXPECT().GetThemeTranslation(ctx, themeName, language).Return(emptyJSON, nil)
		translation, err := managementComponent.GetThemeTranslation(ctx, realmName, language)
		assert.Nil(t, err)
		assert.Equal(t, map[string]interface{}{}, translation)
	})
}
