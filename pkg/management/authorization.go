...
func (c *authorizationComponentMW) GetIdentityProvider(ctx context.Context, realmName string, providerID string) (api.IdentityProviderRepresentation, error) {
	var action = security.MGMTGetIdentityProvider.String()
	var targetRealm = realmName
	if err := c.authManager.CheckAuthorizationOnTargetRealm(ctx, action, targetRealm); err != nil {
		return api.IdentityProviderRepresentation{}, err
	}

	return c.next.GetIdentityProvider(ctx, realmName, providerID)
}
...
func (c *authorizationComponentMW) CreateIdentityProvider(ctx context.Context, realmName string, provider api.IdentityProviderRepresentation) error {
	var action = security.MGMTCreateIdentityProvider.String()
	var targetRealm = realmName
	if err := c.authManager.CheckAuthorizationOnTargetRealm(ctx, action, targetRealm); err != nil {
		return err
	}
...
func (c *authorizationComponentMW) UpdateIdentityProvider(ctx context.Context, realmName string, providerID string, provider api.IdentityProviderRepresentation) error {
	var action = security.MGMTUpdateIdentityProvider.String()
	var targetRealm = realmName
	if err := c.authManager.CheckAuthorizationOnTargetRealm(ctx, action, targetRealm); err != nil {
		return err
	}

	return c.next.UpdateIdentityProvider(ctx, realmName, providerID, provider)
}

func (c *authorizationComponentMW) DeleteIdentityProvider(ctx context.Context, realmName string, providerID string) error {
	var action = security.MGMTDeleteIdentityProvider.String()
	var targetRealm = realmName
	if err := c.authManager.CheckAuthorizationOnTargetRealm(ctx, action, targetRealm); err != nil {
		return err
	}

	return c.next.DeleteIdentityProvider(ctx, realmName, providerID)
}
