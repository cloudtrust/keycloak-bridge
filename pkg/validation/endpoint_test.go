...
func TestUpdateUserAccreditationEndpoint(t *testing.T) {
	...
	t.Run("No error", func(t *testing.T) {
		...
		var req = map[string]string{prmRealm: realm, prmUserID: userID, reqBody: string(accreditationsJSON), prmSponsors: sponsors}

		mockComponent.EXPECT().UpdateUserAccreditations(ctx, realm, userID, accreditations, &sponsors).Return(nil)
		var res, err = e(ctx, req)
		assert.Nil(t, err)
		assert.Nil(t, res)
	})
