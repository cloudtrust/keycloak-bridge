...
func TestCheckUserConsent(t *testing.T) {
	...
	t.Run("Load realm admin config fails", func(t *testing.T) {
		...
		var err = component.checkUserConsent(ctx, accessToken, userID, true, &consentCode)
		assert.NotNil(t, err)
	})
	t.Run("Consent not required", func(t *testing.T) {
		mocks.configDB.EXPECT().GetAdminConfiguration(ctx, realm).Return(configuration.RealmAdminConfiguration{ConsentRequiredSocial: ptrBool(false)}, nil)
		var err = component.checkUserConsent(ctx, accessToken, userID, true, &consentCode)
		assert.Nil(t, err)
	})

	mocks.configDB.EXPECT().GetAdminConfiguration(ctx, realm).Return(configuration.RealmAdminConfiguration{ConsentRequiredSocial: ptrBool(true)}, nil).AnyTimes()

	t.Run("Consent required but not provided", func(t *testing.T) {
		var err = component.checkUserConsent(ctx, accessToken, userID, true, nil)
		assert.NotNil(t, err)
...
func TestSendSmsConsentCode(t *testing.T) {
	...
	mocks.configDB.EXPECT().GetAdminConfiguration(ctx, targetRealm).Return(configuration.RealmAdminConfiguration{ConsentRequiredSocial: ptrBool(true)}, nil).AnyTimes()
	...
	t.Run("Corporate SendSmsConsentCode-Consent is not enabled", func(t *testing.T) {
		mocks.configDB.EXPECT().GetAdminConfiguration(gomock.Any(), targetRealm).Return(configuration.RealmAdminConfiguration{}, nil)
		err := component.SendSmsConsentCode(ctx, targetRealm, userID)
		assert.NotNil(t, err)
	})
	mocks.configDB.EXPECT().GetAdminConfiguration(ctx, targetRealm).Return(configuration.RealmAdminConfiguration{ConsentRequiredCorporate: ptrBool(true)}, nil).AnyTimes()
