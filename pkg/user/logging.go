...
func (m *componentLoggingMW) GetUsers(ctx context.Context, req *fb.GetUsersRequest) (*fb.GetUsersResponse, error) {
	defer func(begin time.Time) {
		m.logger.Log("unit", "user", "realm", string(req.Realm()), LoggingCorrelationIDKey, ctx.Value(CorrelationIDKey).(string), "took", time.Since(begin))
	}(time.Now())
...
func (m *moduleLoggingMW) GetUsers(ctx context.Context, realm string) ([]string, error) {
	defer func(begin time.Time) {
		m.logger.Log("unit", "user", "realm", realm, LoggingCorrelationIDKey, ctx.Value(CorrelationIDKey).(string), "took", time.Since(begin))
	}(time.Now())
