...
type ThemeConfiguration struct {
	Settings *configuration.ThemeConfigurationSettings `json:"settings,omitempty"`
...
func (themeConf UpdatableThemeConfiguration) Validate() error {
	...
	return validation.NewParameterValidator().
		ValidateParameterRegExp(constants.ThemeName, themeConf.ThemeName, constants.RegExpTheme, true).
		...
		ValidateParameterMaxSize(constants.Logo, themeConf.Logo, maxLogoImageSize, true).
		ValidateParameterImageMimeType(constants.Logo, themeConf.Logo, allowedLogoImageTypes, true).
		ValidateParameterMaxSize(constants.Favicon, themeConf.Favicon, maxFaviconImageSize, true).
		ValidateParameterImageMimeType(constants.Favicon, themeConf.Favicon, allowedFaviconImageTypes, true).
		ValidateParameterMaxSize(constants.Translations, &translationsJSON, maxTranslationsSize, true).
		ValidateParameterOnlyStrings(constants.Translations, themeConf.Translations, true).
		Status()
}

func getImageMimeType(imageData []byte) (string, error) {
	if len(imageData) == 0 {
		return "", errorhandler.CreateBadRequestError("image data is required")
	}
...
func ConvertToThemeConfiguration(themeConf configuration.ThemeConfiguration) ThemeConfiguration {
	...
	if themeConf.Logo != nil {
		mimeType, err := getImageMimeType(*themeConf.Logo)
		if err != nil {
			return ThemeConfiguration{Settings: themeConf.Settings}
		}
