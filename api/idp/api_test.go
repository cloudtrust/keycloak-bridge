...
func TestValidateIdentityProviderRepresentation(t *testing.T) {
	...
	var items []IdentityProviderRepresentation
	for range 5 {
		items = append(items, createTestAPIIdp())
	}
	items[0].Alias = ptr("`!not a valid alias!`")
	items[1].InternalID = ptr("not an ID")
	items[2].Config = map[string]string{"singleSignOnServiceUrl": strings.Repeat("A", 1000000)}
	items[3].HrdSettings.IPRangesList = "not a list of IP ranges"
	items[4].HrdSettings.Priority = -250
